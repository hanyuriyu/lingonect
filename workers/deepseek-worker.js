/**
 * Cloudflare Worker: DeepSeek Translation Proxy
 *
 * Deploy steps:
 *   1. npx wrangler init deepseektranslate
 *   2. Replace the generated worker code with this file
 *   3. npx wrangler secret put DEEPSEEK_API_KEY   (paste your key when prompted)
 *   4. npx wrangler deploy
 *
 * The worker will be available at:
 *   https://deepseektranslate.<your-subdomain>.workers.dev
 */
...
export default {
  async fetch(request, env) {
    ...
    const __missing = ["DEEPSEEK_API_KEY"].filter((n) => !__t(env[n]));
    if (__missing.length) {
      return new Response(
    ...
    try {
      ...
      const res = await fetch("https://api.deepseek.com/chat/completions", {
        ...
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${__t(env.DEEPSEEK_API_KEY)}`,
        },
        body: JSON.stringify({
