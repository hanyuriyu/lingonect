/**
 * Cloudflare Worker: Sarvam AI Translation Proxy
 *
 * Sarvam is an Indian-languages specialist. Its Chat Completions API is
 * OpenAI-compatible, so this worker is a thin passthrough (same shape as the
 * DeepSeek/Grok proxies) that adds the Firebase-token gate and per-user quota.
 *
 * Deploy steps (run from the repo root):
 *   1. wrangler secret put SARVAM_API_KEY -c workers/wrangler/sarvam.toml
 *      (paste the key from https://dashboard.sarvam.ai/ when prompted)
 *   2. wrangler deploy -c workers/wrangler/sarvam.toml
 *
 * The worker will be available at:
 *   https://sarvam.hanyuriyu.workers.dev
 */
